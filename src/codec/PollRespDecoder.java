...
public class PollRespDecoder extends LSNDecoder{

	private static Logger LOG = LoggerFactory.getLogger(PushDecoder.class);

	@Override
	public MessageDecoderResult decodable(IoSession session, IoBuffer in) {
		return Utils.checkDecodable(AbstractMessage.POLLRESP, in);
	}
	...
	public MessageDecoderResult decode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
        ...
        //固定头
        PollRespMessage message = new PollRespMessage();
        if (decodeCommonHeader(message, in) == NEED_DATA) {
            LOG.info("POLL-需要更多数据才能解码 " + in);
            return NEED_DATA;
        }
